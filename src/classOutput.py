...
class ImpactSubtypes(BaseModel):
    ...
    def validate_impact_subtype(cls, value):
        if not isinstance(value, list):  # or isinstance(value, type(None)):
            raise ValueError(
                f"Expected a list or NoneType for impactSubtypes, got {type(value).__name__}"
            )
        invalid_types = [
            subtypes for subtypes in value if subtypes not in cls._impactSubtypes_list
        ]
        if invalid_types:
            raise ValueError(
                f"Invalid impactSubtypes: {invalid_types}. Must be one or more of {cls._impactSubtypes_list}"
            )
        return value
...
class ImpactDatesMultiple(BaseModel):
    """For qualitative impacts: multiple start-end date pairs with individual annotations"""

    datePairs: List[DatePair]

    @field_validator("datePairs")
    def validate_date_pairs(cls, value):
        if not value or len(value) == 0:
            raise ValueError("Must provide at least one date pair")
        return value
...
class ImpactHazards(BaseModel):
    ...
    def validate_hazards(cls, value):
        ...
        invalid_hazards = [
            hazard for hazard in value if hazard not in cls._hazardTypes_list
        ]
        if invalid_hazards:
            if cls._validate_hazards:
                raise ValueError(
                    f"Invalid hazards: {invalid_hazards}. Must be one of {cls._hazardTypes_list}"
                )
            else:
                print(
                    f"Invalid hazards: {invalid_hazards}. Must be one of {cls._hazardTypes_list}"
...
class ImpactDetailTypeSubType(ImpactDetail):
    ...
    def set_allowed_classes(
        ...
        impact_subtypes: List[str],
        hazard_types: Union[None, List[str]],
    ):
    ...
    def validate_impact_type(cls, value):
        if value not in cls._impactTypes_list:
            raise ValueError(
                f"Invalid impactType: {value}. Must be one of {cls._impactTypes_list}"
            )
        return value
...
class ImpactDetailConstUnit(ImpactDetail):
    ...
    def set_allowed_classes(
        ...
        impact_types: List[str],
        impact_subtypes: List[str],
        impact_units: Union[None, List[str]],
    ...
    def validate_impact_unit(cls, value):
        if value and value not in cls._impactUnits_list:
            raise ValueError(
                f"Invalid impactUnit: {value}. Must be one of {cls._impactUnits_list}"
            )
        return value
