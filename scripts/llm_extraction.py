...
file_path = DATA_IN_JSONS / "preproc_filtered_report_types_nat_hazards_bugfix_v100925.csv"#'all_ifrc_reports_info_processed_extended_format_nb_std_units.json' "nathaz_ifrc_reports_info_processed.json"
ifrc_reports_df = pd.read_csv(file_path)

# eventually load labelled reports
labelled_reports = pd.read_csv(DATA_LABELLED / "labelled_reports_impacts_all_v080925.csv")
...
nreports = 2
reports_in = labelled_reports_raw[:nreports] #ifrc_reports_df.iloc[:nreports] test_reports
nreports = len(reports_in)


## Parameters
sim_name = "test_script"
...
impsubtype = list(impsubtype_dict.keys())
